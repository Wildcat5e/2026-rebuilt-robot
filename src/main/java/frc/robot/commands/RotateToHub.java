...
public class RotateToHub extends Command {
    ...
    public void execute() {
        ...
        double outputSpeeds = PID_CONTROLLER.calculate(currentPose.getRotation().getRadians(), robotToHubAngle);
        if (counter % 20 == 0) { // debug
            // System.out.println(currentPose.getRotation());
            // System.out.printf("robotToHubAngle(Rads: %.2f, Deg: %.2f)\n", robotToHubAngle, Math.toDegrees(robotToHubAngle);
        }
